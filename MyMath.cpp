...
//	const float kLonEvery = PAI * 2.0f / float(kSubdivision);
//	const float kLatEvery = PAI / float(kSubdivision);
//
//	for (uint32_t latIndex = 0; latIndex < kSubdivision; ++latIndex) {
//		float lat = -PAI / 2.0f + kLatEvery * latIndex;
//		for (uint32_t lonIndex = 0; lonIndex < kSubdivision; ++lonIndex) {
//			uint32_t startIndex = (latIndex * kSubdivision + lonIndex) * 6;
//			float lon = lonIndex * kLonEvery;
//
//			vertexData[startIndex].position.x = cosf(lat) * cosf(lon); 
//			vertexData[startIndex].position.y = sinf(lat);
//			vertexData[startIndex].position.z = cosf(lat) * sinf(lon);
